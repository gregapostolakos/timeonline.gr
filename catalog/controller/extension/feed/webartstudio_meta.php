<?php
/**
 * Meta Catalog XML Feed — catalog controller (the XML generator).
 *
 * Produces a Google Shopping RSS 2.0 feed on the fly at:
 *   index.php?route=extension/feed/webartstudio_meta   (or the clean "mask" URL)
 *
 * Site-agnostic: all behaviour is driven by the feed_webartstudio_meta_* settings.
 */
class ControllerExtensionFeedWebartstudioMeta extends Controller {

	// Google product-level availability vocabulary.
	private const AVAILABILITY = ['in stock', 'out of stock', 'available for order', 'preorder', 'discontinued'];

	public function index(): void {
		if (!$this->config->get('feed_webartstudio_meta_status')) {
			$this->sendNotFound();
			return;
		}
		...
		$products = $this->model_extension_feed_webartstudio_meta->getFeedProducts($s);
		...
		$writer->writeElement('title', $this->config->get('config_name') ?: 'Meta Catalog Feed');
		$writer->writeElement('link', (defined('HTTPS_SERVER') ? HTTPS_SERVER : HTTP_SERVER));
		$writer->writeElement('description', 'Meta product catalog feed');
		...
		foreach ($products as $p) {
			...
			$special = $this->model_extension_feed_webartstudio_meta->getSpecialPrice($product_id, (int)$s['customer_group_id']);
			$sale_price = null;
			...
			$product_type = $this->model_extension_feed_webartstudio_meta->getCategoryPath($product_id, (int)$s['language_id']);
			...
			if (!empty($s['additional_images'])) {
				foreach ($this->model_extension_feed_webartstudio_meta->getAdditionalImages($product_id, 10) as $img) {
		...
		for ($i = 0; $i < 5; $i++) {
			...
			foreach ($s['label_rules'] as $rule) {
				...
				if ($rule['type'] === 'category') {
					if ($path_ids === null) {
						$path_ids = $this->model_extension_feed_webartstudio_meta->getProductCategoryPathIds($product_id);
					}
	...
	private function labelFromSource(string $source, array $p, array $ctx, array $s): string {
		...
		switch ($source) {
			...
			case 'category':
				return $this->model_extension_feed_webartstudio_meta->getTopCategoryName($product_id, (int)$s['language_id']);

			case 'category_last':
				return $this->model_extension_feed_webartstudio_meta->getLastCategoryName($product_id, (int)$s['language_id']);

			case 'category_path':
				return $this->model_extension_feed_webartstudio_meta->getCategoryPath($product_id, (int)$s['language_id']);
		...
		if ($type === 'attribute') {
			$values = $this->model_extension_feed_webartstudio_meta->getAttributeValues($product_id, $id, (int)$s['language_id']);
			return $values[0] ?? '';
		}
		if ($type === 'option') {
			$rows = $this->model_extension_feed_webartstudio_meta->getOptionSizes($product_id, $id, (int)$s['language_id']);
			return $rows[0]['name'] ?? '';
		}
		return '';
	}
	...
	private function getSizes(int $product_id, array $s): array {
		...
		if ($type === 'option') {
			foreach ($this->model_extension_feed_webartstudio_meta->getOptionSizes($product_id, $id, (int)$s['language_id']) as $row) {
			...
			foreach ($this->model_extension_feed_webartstudio_meta->getAttributeValues($product_id, $id, (int)$s['language_id']) as $value) {
	...
	private function applyBuffer(float $net, int $product_id, int $manufacturer_id, array $buffers, array $general): float {
		...
		if ($buffers['category']) {
			foreach ($this->model_extension_feed_webartstudio_meta->getProductCategories($product_id) as $cat) {
	...
	private function buildSettings(): array {
		$instock = $this->config->get('feed_webartstudio_meta_availability_instock') ?: 'in stock';
		if (!in_array($instock, self::AVAILABILITY, true)) {
			$instock = 'in stock';
		}

		return [
			'store_id'             => (int)$this->config->get('feed_webartstudio_meta_store_id'),
			'language_id'          => (int)($this->config->get('feed_webartstudio_meta_language_id') ?: $this->config->get('config_language_id')),
			'customer_group_id'    => (int)($this->config->get('feed_webartstudio_meta_customer_group_id') ?: $this->config->get('config_customer_group_id')),
			'currency'             => strtoupper($this->config->get('feed_webartstudio_meta_currency') ?: 'EUR'),
			'weight_unit'          => $this->config->get('feed_webartstudio_meta_weight_unit') ?: 'g',
			'mpn_field'            => $this->config->get('feed_webartstudio_meta_mpn_field') ?: 'mpn',
			'ean_field'            => $this->config->get('feed_webartstudio_meta_ean_field') ?: 'ean',
			'desc_source'          => $this->config->get('feed_webartstudio_meta_desc_source') ?: 'description',
			'vat_default'          => $this->config->get('feed_webartstudio_meta_vat_default') !== null ? (float)$this->config->get('feed_webartstudio_meta_vat_default') : 24.0,
			'strip_html'           => $this->config->get('feed_webartstudio_meta_strip_html'),
			'variations'           => $this->config->get('feed_webartstudio_meta_variations'),
			'size_source'          => $this->config->get('feed_webartstudio_meta_size_source') ?: 'none',
			'color_source'         => $this->config->get('feed_webartstudio_meta_color_source') ?: 'none',
			'additional_images'    => $this->config->get('feed_webartstudio_meta_additional_images'),
			'condition'            => $this->config->get('feed_webartstudio_meta_condition') ?: 'new',
			'gender'               => (string)$this->config->get('feed_webartstudio_meta_gender'),
			'age_group'            => (string)$this->config->get('feed_webartstudio_meta_age_group'),
			'google_category'      => (string)$this->config->get('feed_webartstudio_meta_google_category'),
			'availability_instock' => $instock,
			'availability_map'     => (array)$this->config->get('feed_webartstudio_meta_availability_map'),
			'exclude_stock'        => (array)$this->config->get('feed_webartstudio_meta_exclude_stock'),
			'exclude_empty'        => (array)$this->config->get('feed_webartstudio_meta_exclude_empty'),
			'buffer_op'            => $this->config->get('feed_webartstudio_meta_buffer_op') ?: 'percent',
			'buffer_val'           => (float)$this->config->get('feed_webartstudio_meta_buffer_val'),
			'buffer_round'         => $this->config->get('feed_webartstudio_meta_buffer_round'),
			'shipping'             => (string)$this->config->get('feed_webartstudio_meta_shipping'),
			'shipping_country'     => $this->config->get('feed_webartstudio_meta_shipping_country') ?: 'GR',
	...
	private function labelSources(): array {
		$out = [];
		for ($i = 0; $i < 5; $i++) {
			$out[$i] = (string)($this->config->get('feed_webartstudio_meta_label_' . $i . '_source') ?: 'none');
		}
		return $out;
	}
	...
	private function priceBuckets(): array {
		$raw = (string)$this->config->get('feed_webartstudio_meta_price_buckets');
		$buckets = [];
