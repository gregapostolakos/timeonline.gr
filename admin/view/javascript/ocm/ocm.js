...
function addToMethodList(id, name) {
    ...
    var tab = '<li draggable="true" class="nav-item draggable">'
                 + '<a draggable="false" class="nav-link" ' + getDataPrefix() + '-toggle="tab" href="#ocm-method-' + id + '" rel="' + id + '">' + name + '</a>'
...
function enableEvents(no_of_tab){
    ...
    $("[" + getDataPrefix() + "-toggle='tooltip']", ocm_method).tooltip();
    $('input.ocm-autofill', ocm_method).autocomplete(ocm.complete);
    if (!_ocm.oc4 && $.fn.datetimepicker) {
...
function getDataPrefix() {
  ...
  if (typeof bootstrap !== "undefined" && bootstrap.Tooltip) {
    version = bootstrap.Tooltip.VERSION; // For Bootstrap 5+
  } else if (typeof $.fn !== "undefined" && $.fn.tooltip) {
    version = $.fn.tooltip.Constructor.VERSION; // For Bootstrap 4 and earlier
  } else {
    return _ocm.oc4 ? 'data-bs' : 'data';
  }
  const majorVersion = parseInt(version.split('.')[0], 10);
  if (majorVersion >= 5) {
    return "data-bs"; // Bootstrap 5+
  } else {
    return "data"; // Bootstrap 4 and earlier
  }
}
