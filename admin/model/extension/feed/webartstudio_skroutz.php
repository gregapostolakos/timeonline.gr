 ...
 * (including the exclude/buffer lists) is stored as JSON in oc_setting.
 */
class ModelExtensionFeedWebartstudioSkroutz extends Model {
	...
	public function uninstall(): void {
		// Remove every stored setting and the SEO keyword (the "mask").
		$this->db->query("DELETE FROM `" . DB_PREFIX . "setting` WHERE `code` = 'feed_webartstudio_skroutz'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "seo_url` WHERE `query` = 'extension/feed/webartstudio_skroutz'");
	}
	...
	/**
	 * Map a clean keyword (e.g. "skroutz") to the feed route for every
	 * store/language, so the feed is reachable at https://domain/<keyword>.
	 * Passing an empty keyword just clears any existing mapping.
	 */
	public function setMask(string $keyword): void {
		...
		// One mapping per language is enough; OC resolves regardless of store_id 0.
		foreach ($this->getLanguages() as $language) {
			$this->db->query("
				INSERT INTO `" . DB_PREFIX . "seo_url`
				SET `store_id`    = '0',
					`language_id` = '" . (int)$language['language_id'] . "',
					`query`       = 'extension/feed/webartstudio_skroutz',
	...
	public function getMask(): string {
		$query = $this->db->query("
			...
			WHERE `query` = 'extension/feed/webartstudio_skroutz' LIMIT 1
		");
