<?php
/**
 * Meta Catalog XML Feed — admin model.
 *
 ...
 */
class ModelExtensionFeedWebartstudioMeta extends Model {
	...
	public function uninstall(): void {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "setting` WHERE `code` = 'feed_webartstudio_meta'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "seo_url` WHERE `query` = 'extension/feed/webartstudio_meta'");
	}
		...
		foreach ($this->getLanguages() as $language) {
			$this->db->query("
				...
				SET `store_id`    = '0',
					...
					`query`       = 'extension/feed/webartstudio_meta',
	...
	public function getMask(): string {
		$query = $this->db->query("
			...
			WHERE `query` = 'extension/feed/webartstudio_meta' LIMIT 1
		");
