...
class ControllerExtensionPaymentPayPal extends Controller
{
	...
	public function __construct($registry)
	{
		...
		$paypal_version = (string)$this->config->get('paypal_version');

		// Check PayPal version and update if needed
		if ($paypal_version === '' || version_compare($paypal_version, '3.1.4', '<')) {
			$this->update();
		}
	}
	...
	public function update()
	{
		...
		if ($paypal_version === '' || version_compare($paypal_version, '3.1.0', '<')) {
			$this->load->model('setting/setting');
