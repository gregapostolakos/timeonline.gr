...
final class Range extends Base {
    ...
    private function getDate($params) {
        // todo - to use html calender for all version.
        if (VERSION > '4.0.0.0') {
            $start = '<input type="date" name="' . $this->getSymbolicName($params['name'].'_start'). '" value="' . $params['preset_start'] . '" placeholder="{text_start}" class="form-control" />';
            $end = '<input type="date" name="' . $this->getSymbolicName($params['name'].'_end'). '" value="' . $params['preset_end'] . '" placeholder="{text_end}" class="form-control" />';
        } else {
