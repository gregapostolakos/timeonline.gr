...
final class Misc {
    ...
    public function __construct($registry, $common, $meta) {
        ...
        $this->common = $common;
        $this->ocm_meta = $meta;
        $this->data_attr = $this->common->getCustomConfig("dataPrefix");
    ...
    // versionize the string. 
    public function stringVersionize($str) {
        // apply data-attribute-prefix 
        $expectedVersionDataPrefix = VERSION >= '4.0.0.0' ? 'data-' : 'data-';
        if ($this->data_attr != $expectedVersionDataPrefix && strpos($str, $this->data_attr) === false) {
            $str = str_replace($expectedVersionDataPrefix, $this->data_attr, $str);
        }
        return $str;
    }

    public function arrayVersionize($data, $keys) {
        foreach($keys as $key) {
            if (!empty($data[$key])) {
                $data[$key] = $this->stringVersionize($data[$key]);
            }
        } 
        return $data;
    }
