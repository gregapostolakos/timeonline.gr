...
final class Util {
    ...
    public function addEvents($events, $code = '') {
        ...
        foreach ($events as $event) {
            ...
            if (strpos($event['trigger'], 'controller') !== false) {
                $event['trigger'] = $this->applyMethodDivider($event['trigger']); // replace oc4 . with other oc version compatible
            }
            if (VERSION == '4.0.0.0') {
                $description = '';
            ...
            if ($this->isEventChanged($this->ocm_events, $existing_events)) {
                ...
                foreach ($this->ocm_events as $event) {
                    ...
                    if (VERSION == '4.0.0.0') {
                        ...
                        $data = array();
                        $data['code'] = 'ocm';
                        $data['description'] = 'Core events for OpenCartMart';
                        $data['trigger'] = $event['trigger'];
                        $data['action'] = $event['action'];
                        $data['status'] = true;
                        $data['sort_order'] = 1;
    ...
    private function applyMethodDivider($input) {
        $method_divider = '.';
        if (VERSION >= '4.0.0.0' && VERSION < '4.0.2.0') {
             $method_divider = '|';
        }
        if (VERSION < '4.0.0.0') {
             $method_divider = '/';
        }
        return str_replace('.', $method_divider, $input);
    }
}